...
class GenerationMixin(ContinuousMixin):
    ...
    `GenerationConfig` at initialization time or ensuring `generate`-related small_tests are run in `transformers` CI.
    ...
    def generate(
        ...
        **kwargs,
        ...
        # - diffeFrent models have a different cache name expected by the model (default = "past_key_values")
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            model_kwargs = self._update_model_kwargs_for_generation(
                ...
                next_step_suffix_len=suffix_eval_ids.shape[1] if should_evaluate_next else 0,
                is_encoder_decoder=self.config.is_encoder_decoder,
            )
            if synced_gpus and this_peer_finished:
                continue
