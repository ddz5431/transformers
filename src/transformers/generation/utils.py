...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    suffix_eval_results: Optional[list] = None
...
class GenerationMixin:
    ...
    def _build_analyzer(self, tokenizer, input_ids, eval_input_ids, full_input_ids, generation_config):
    ...
    def _should_skip_batch_samples(self, analyzer, save_results):
        ...
        for batch_idx in range(analyzer.batch_size):
            skip_this, file_path = self._should_skip_individual_sample(analyzer, batch_idx)

            if skip_this:
                logger.info(f"Skipping batch item {batch_idx}, file already exists: {file_path}")
            else:
                skip_all = False
    ...
    def _should_skip_individual_sample(self, analyzer, batch_idx):
        try:
            ...
            logger.error(f"Error checking if file exists for batch item {batch_idx}: {e}")
            return False, None

    def _self_align_decoding(
        self,
        input_ids: torch.LongTensor,
        eval_input_ids: torch.LongTensor,
        ...
        **model_kwargs,
        ...
        Parameters:
            ...
            eval_input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
                The sequence used as a suffix for checking the generation.
            logits_processor (`LogitsProcessorList`):
        ...
        batch_size, cur_len = full_input_ids.shape
        this_peer_finished = False
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=full_input_ids.device)
        ...
        analyzer = self._build_analyzer(tokenizer, input_ids, eval_input_ids, full_input_ids, generation_config)
        ...
        while self._has_unfinished_sequences(
            ...
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs,
                model_kwargs,
                num_eval_input_ids=eval_input_ids.shape[1],
            ...
            analyzer.add_decoding_step(eval_logits, full_input_ids, generated_token_id)
            ...
            generated_token_id = next_tokens[0].item()
    ...
    def _update_model_kwargs_for_generation(
        ...
        if suffix_len > 0:
            # Suffix case: sliding window logic (same for both use_cache True/False)
            suffix_start_pos = model_kwargs["cache_position"][-suffix_len]
            model_kwargs["cache_position"] = suffix_start_pos + torch.arange(
                suffix_len + 1,
                device=model_kwargs["cache_position"].device
            )
        elif model_kwargs.get("use_cache", True):
            # Normal generation WITH cache: just increment
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Extract suffix evaluation parameters if provided
        suffix_eval_ids = model_kwargs.pop("suffix_eval_ids", None)
        logit_analyzer = model_kwargs.pop("logit_analyzer", None)
    ...
    def _generate_with_efficient_self_evaluation(
        self,
        input_ids: torch.LongTensor,
        logits_processor: LogitsProcessorList,
        stopping_criteria: StoppingCriteriaList,
        ...
        synced_gpus: bool,
        streamer: Optional["BaseStreamer"],
        suffix_eval_ids: torch.LongTensor,
        logit_analyzer: Optional["LogitAnalyzer"],
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, torch.LongTensor]:
        # init values
        pad_token_id = generation_config._pad_token_tensor
        ...
        batch_size, cur_len_with_eval = input_ids_with_eval.shape[:2]
        cur_len = input_ids.shape[-1]
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_kwargs = self._get_initial_cache_position(cur_len_with_eval, input_ids_with_eval.device, model_kwargs)
