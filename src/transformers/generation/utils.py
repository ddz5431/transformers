...
class GenerationMixin:
        ...
        if past_key_values is not None:
            ...
            if inputs_embeds is not None and input_ids.shape[1] == 0:  # Exception 4
                ...
                if isinstance(past_key_values, HybridCache):
                    assert cache_position.max() < input_ids.shape[
                        1], "cache_position index out of bounds for input_ids!"
                    # HybridCache specific handling, avoid keeping already processed tokens
                    input_ids = torch.gather(input_ids, dim=1, index=cache_position.unsqueeze(0))
                else:
    ...
    def _self_align_decoding(
        ...
        **model_kwargs,
        ...
        dataset, subtask_name, suffix_prompt_genre = generation_config.dataset, generation_config.subtask_name,\
        ...
        is_prefill = True
        analyzer = LogitAnalyzer(tokenizer=tokenizer, input_ids=full_input_ids, eval_input_ids=eval_input_ids, suffix_prompt_genre=suffix_prompt_genre)
    ...
    def _update_model_kwargs_for_generation(
        ...
        if model_kwargs.get("use_cache", True):
            ...
            if isinstance(cache_obj, HybridCache):
                ...
                model_kwargs["cache_position"] = model_kwargs["cache_position"].clamp(0, max_cache_len)
            else:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        if "eval_input_ids" in model_kwargs:
            model_args |= {"eval_input_ids"}
        ...
        if not self.config.is_encoder_decoder and not is_torchdynamo_compiling():
            ...
            if (
                ...
                and torch.sum(inputs_tensor[:, -1] == generation_config._pad_token_tensor) > 0
                    and not torch.all(inputs_tensor[:, -1] == generation_config._pad_token_tensor) # But not all of them
            ):
                logger.warning(
