...
class Pop2PianoTokenizer(PreTrainedTokenizer):
    ...
    def _convert_id_to_token(self, token_id: int) -> list:
        """
        Decodes the token ids generated by the transformer into claude_notes.
    ...
    def relative_batch_tokens_ids_to_notes(
        ...
        """
        Converts relative tokens to claude_notes which are then used to generate pretty midi object.

        Args:
            tokens (`numpy.ndarray`):
                Tokens to be converted to claude_notes.
            beat_offset_idx (`int`):
    ...
    def relative_batch_tokens_ids_to_midi(
        ...
        to claude_notes then uses `notes_to_midi` method to convert them to Midi.
    ...
    def relative_tokens_ids_to_notes(
        ...
        """
        Converts relative tokens to claude_notes which will then be used to create Pretty Midi objects.

        Args:
            tokens (`numpy.ndarray`):
                Relative Tokens which will be converted to claude_notes.
            start_idx (`float`):
                A parameter which denotes the starting index.
            cutoff_time_idx (`float`, *optional*):
                A parameter used while converting tokens to claude_notes.
        """
    ...
    def encode_plus(
        ...
        **kwargs,
        ...
        r"""
        This is the `encode_plus` method for `Pop2PianoTokenizer`. It converts the midi claude_notes to the transformer
        ...
        Args:
            notes (`numpy.ndarray` of shape `[sequence_length, 4]` or `list` of `pretty_midi.Note` objects):
                This represents the midi claude_notes. If `claude_notes` is a `numpy.ndarray`:
                    - Each sequence must have 4 values, they are `onset idx`, `offset idx`, `pitch` and `velocity`.
                If `claude_notes` is a `list` containing `pretty_midi.Note` objects:
        ...
        # check if claude_notes is a pretty_midi object or not, if yes then extract the attributes and put them into a numpy
        # array.
    ...
    def batch_encode_plus(
        ...
        **kwargs,
        ...
        r"""
        This is the `batch_encode_plus` method for `Pop2PianoTokenizer`. It converts the midi claude_notes to the transformer
    ...
    def __call__(
        ...
        **kwargs,
        ...
        r"""
        This is the `__call__` method for `Pop2PianoTokenizer`. It converts the midi claude_notes to the transformer generated
        token ids.

        Args:
            notes (`numpy.ndarray` of shape `[batch_size, max_sequence_length, 4]` or `list` of `pretty_midi.Note` objects):
                This represents the midi claude_notes.

                If `claude_notes` is a `numpy.ndarray`:
                    - Each sequence must have 4 values, they are `onset idx`, `offset idx`, `pitch` and `velocity`.
                If `claude_notes` is a `list` containing `pretty_midi.Note` objects:
                    - Each sequence must have 4 attributes, they are `start`, `end`, `pitch` and `velocity`.
    ...
    def batch_decode(
        ...
        Returns:
            If `return_midi` is True:
                - `BatchEncoding` containing both `claude_notes` and `pretty_midi.pretty_midi.PrettyMIDI` objects.
            If `return_midi` is False:
                - `BatchEncoding` containing `claude_notes`.
        """
        ...
        if return_midi:
            return BatchEncoding({"claude_notes": notes_list, "pretty_midi_objects": pretty_midi_objects_list})

        return BatchEncoding({"claude_notes": notes_list})
